...
public class Prof_GUI extends JFrame implements ActionListener{
	...
	public Prof_GUI(String name, String pw,Socket socket) {
        ...
        label2=new JLabel("欢迎使用教师选课系统！");//显示当前学期
		...
		if(e.getSource()==jb1) {  //选择执教课程功能
			...
			try {
				...
				//不管是否在注册时间，下面这些都要执行
					...
					{
						JOptionPane.showMessageDialog(null,"本学期注册已结束，不能更改教授课程信息，如果注册结束后需要更换授课教师，请联系教务主任处理","不在注册时间",JOptionPane.INFORMATION_MESSAGE);
						//socket.close();//服务终止
						dispose();
